...
import { Calendar, Download, Calculator, FileSpreadsheet, Settings } from 'lucide-react';
...
function App() {
  ...
  const [appName, setAppName] = useState('RevenuePulse AI');
  const [currentThemeId, setCurrentThemeId] = useState('terra-cotta');
  ...
  const handleManualInputUpdate = (metrics: { id: string; value: number; trend: number }[]) => {
    const generateSyntheticHistory = (val: number, trend: number) => {
        ...
        for (let i = 0; i < points; i++) {
            ...
            // Add some "noise" so lines aren't perfectly flat/straight for better visuals
            const noise = (Math.random() - 0.5) * (val * 0.05); 
            const currentV = startVal + ((val - startVal) * (i / points)) + noise;
            
            history.push({
                date: d.toISOString().split('T')[0],
                value: Math.max(0, currentV)
            });
        }
        return history;
    };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col transition-colors duration-500">
      ...
      <style>{`
        :root {
          ...
          --theme-high-risk: ${currentTheme.colors.highRisk};
          --theme-medium-risk: ${currentTheme.colors.mediumRisk};
          --theme-low-risk: ${currentTheme.colors.lowRisk};
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
             {/* Logo remains neutral - does not change color */}
            <Logo size={36} />
            <h1 className="text-xl font-bold transition-colors" style={{ color: currentTheme.colors.text }}>{appName}</h1>
          </div>
        ...
        <div className="border-b border-slate-200 mb-6">
            <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                <button
                    ...
                    style={{
                        borderColor: inputMode === 'manual' ? currentTheme.colors.primary : 'transparent',
                        color: inputMode === 'manual' ? currentTheme.colors.primary : '#64748b'
                    }}
                >
                ...
                <button
                    ...
                    className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center transition-colors`}
                    style={{
                        borderColor: inputMode === 'import' ? currentTheme.colors.primary : 'transparent',
                        color: inputMode === 'import' ? currentTheme.colors.primary : '#64748b'
                    }}
                >
             ...
             <div className="flex flex-col sm:flex-row items-center gap-2 bg-white p-1 rounded-lg border border-slate-200 shadow-sm w-auto">
                {(['3m', '6m', '9m', '1y'] as DateRangeType[]).map((t) => (
                    <button
                        ...
                        className={`px-3 py-1 text-xs rounded-md font-medium transition-all whitespace-nowrap ${inputMode === 'manual' ? 'opacity-50 cursor-not-allowed' : ''}`}
                        style={{
                            backgroundColor: dateRange.type === t ? `${currentTheme.colors.primary}15` : 'transparent',
                            color: dateRange.type === t ? currentTheme.colors.primary : '#475569'
                        }}
                    >
                        {t}
                    </button>
                ...
                <button
                        ...
                        className={`px-3 py-1 text-xs rounded-md font-medium transition-all whitespace-nowrap ${inputMode === 'manual' ? 'opacity-50 cursor-not-allowed' : ''}`}
                        style={{
                            backgroundColor: dateRange.type === 'custom' ? `${currentTheme.colors.primary}15` : 'transparent',
                            color: dateRange.type === 'custom' ? currentTheme.colors.primary : '#475569'
                        }}
                >
                    Aangepast
                </button>
      ...
      <footer className="border-t border-slate-200 py-6 mt-auto bg-white/50">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center opacity-40 hover:opacity-100 transition-opacity">
            <div className="flex items-center gap-2">
                <Logo size={24} opacity={0.5} />
                <span className="text-xs text-slate-400 font-medium">Powered by {appName}</span>
            </div>
            <div className="text-xs text-slate-400">
                v1.2.0 • Thema: {currentTheme.name}
            </div>
        </div>
      </footer>
